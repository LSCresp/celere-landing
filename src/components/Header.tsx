...
export function Header() {
    ...
    return (
        <header className={`fixed top-0 w-full z-50 transition-all duration-500 border-b border-celere-gold/10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] bg-transparent ${scrolled
            ? "py-2"
            : "py-4 md:py-5"
            }`}>

            {/* Background Dinâmico do Header */}
            <div className={`absolute inset-0 z-[-3] transition-all duration-500 ${
                scrolled || isMenuOpen
                    ? "bg-[#050505]/95 backdrop-blur-xl opacity-100" 
                    : "bg-[#050505]/60 backdrop-blur-md md:bg-transparent md:backdrop-blur-none"
            }`} />

            {/* Background da Imagem (Aparece apenas quando não scrollado e no desktop) */}
            <div
                className={`absolute inset-0 w-full h-full z-[-2] bg-cover bg-center bg-no-repeat md:bg-contain md:bg-repeat-x transition-opacity duration-500 ${scrolled || isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            ...
            <div className={`absolute inset-0 w-full h-full z-[-1] transition-opacity duration-500 ${scrolled || isMenuOpen ? 'opacity-0 pointer-events-none' : 'bg-black/50 opacity-100'}`}></div>
